...
app.post("/at", async (req, res) => {
  const id = await usr.findOne(
    req.headers.role == "0"
      ? { _id: req.headers.pk }
      : { "emplee._id": req.headers.pk }
  );
  console.log(req.body);
  req.body.map(async (v) => {
    await att.updateOne({ id: v }, { $push: { absent: Date.now } });
  });
  const emp = id.emplee.map((v) => {
    if (!req.body.includes(v._id.toString())) return v._id;
  });
  console.log(emp);
  emp.map(async (v) => {
...
app.post("/pi", async (req, res) => {
  if (req.body.role != null && req.body.pk != null) {
    const id = await usr.findOne(
      req.body.role == 0 ? { _id: req.body.pk } : { "emplee._id": req.body.pk }
    );
    res.json({
      name:
        req.body.role == 0
          ? id.name.toUpperCase()
          : id.emplee[fndId(id.emplee, req.body.pk)].name.toUpperCase(),
      role:
        req.body.role == 0
          ? "OWNER"
          : id.emplee[fndId(id.emplee, req.body.pk)].role.toUpperCase(),
    });
  }
});
...
app.get("/e", async (req, res) => {
  const ed = await usr.findOne(
    req.headers.owner == 0
      ? { _id: req.headers.auth }
      : { "emplee._id": req.headers.auth }
  );
...
app.post("/ae", async (req, res) => {
  ...
  const clust = await usr.findOne(
    owner == 0 ? { _id: pk } : { "emplee._id": pk }
  );
  clust &&
    usr
      .updateOne({ _id: clust._id }, { $push: { emplee: req.body } })
      .then(async () => {
        const nClust = await usr.findOne(
          owner == 0 ? { _id: pk } : { "emplee._id": pk }
        );
        await att.create({
          id: nClust.emplee[clust.emplee.length]._id.toString(),
        });
...
app.post("/log", async (req, res) => {
  ...
  if (id) {
    ...
    res.json({
      status: true,
      acc: id.emplee[fndId(id.emplee, pk)].role == "supervisor",
      role: id.emplee[fndId(id.emplee, pk)].role,
    });
  } else {
